...
<details><summary id="ver-44"><h2> MSDK v4.4.0 Release Note </h2></summary>

## ※ 4.4.0.2 - 2024-10-25
### ※ [更新內容]
### Android
- 修正: 修正用戶中心版本號碼字型大小不一致的問題。
- 更新: 手機認證區碼新增輸入關鍵字選擇功能。
- 更新: AdaptiveIcon 作為 AppIcon 的處理方式改回原本方法：AppIcon = foreground + background。
- 更新: 修正 Android 11 以下版本在取得推播權限時會閃退的問題。
- 修正: GoogleBilling 修正第二次初始化呼叫無法接收到初始化結果回調的問題。
- 修正: GoogleBilling / PlayerCharactor 當初始化參數不變時，購買初始化仍會重新執行初始化過程（因為 PlayerCharactor 未儲存資料且已設置）。
### iOS
- 修正: 新增驗證更改密碼時新密碼和確認密碼需相同的判斷。
- 修正: 修正密碼格式錯誤時仍會送出修改密碼請求的問題。
### Unity
- 修正: 更改需進行登入的流程以使用 LoginByHashAccountId。
- 更新: 優化 Unity WebView Demo，並新增需開啟 Pixel Perfect 的設定需求。
- 更新: 放大 UJ 平台的網頁顯示。
- 更新: Http listener 新增回應關閉功能。
### Server
- 更新: 調整傳送資料至 http listener 的方式（1. iframe.onload 事件觸發後重定向，2. 等待 3 秒後直接重定向）。
- 新增: Mars 後台新增查詢 UI 顯示狀態功能。  
- 新增: 新增可設定儲值獎勵在特定時間區間內開啟的功能。
- 新增: 新增並調整 Mars 後台與儲值表的 Discord 通知。  

## ※ 4.4.0.1 - 2024-10-01
### ※ [已知問題]
- [Unreal][Android] target 34包檔, 索取通知權限無反應問題
- [Unreal][iOS] 刪除帳號會導致APP閃退問題

### ※ [更新內容]
### Android
- 調整: 社群登入失敗(GP,FB,APPLE,Twitter..etc)未知錯誤時, 顯示錯誤訊息

### iOS
- 調整: 社群登入失敗(GP,FB,APPLE,Twitter..etc)未知錯誤時, 顯示錯誤訊息
- 修正: 角色註冊API(PlayerCharactor setCharacterData)無法正常使用問題

### Unity
- 調整: [WebUI] 某些操作加上加上Loading畫面, 以減少介面操作沒有反應的感覺
- 修正: [WebUI] 無帳號狀態下可開啟會員中心的問題

### Unreal
> [WebUI] 同步Unity的調整:
- 新功能:新增取得PC版各網頁的縮放比例
- 新功能:調整PC版縮放網頁流程
- 修正:修正PC版粉絲頁，在啟用兩個以上粉絲頁會異常的問題
- 調整:PC版粉絲頁改為外開
- 新功能:InitMSDKWithOS加入bundleName與bundleVersion參數
- 調整:一些操作加上loading畫面，以減少介面操作沒有反應的感覺
- 修正: 無帳號狀態下可開啟會員中心的問題
